...
- **Date:** 2026-09-14
- **Phase:** Phase 8.5.3 complete locally (`requested_model` audit key, point release `0.8.5.3`); deploy, deployed-DB reset and deployed verification pending.
- **Demo status: blocked.** Phase 8.5.2's deployed verification failed on 14 September (run `11f97ae8-7dc6-4b41-9b8e-16c85d4bd073`): `mistral-large-2512` is also `403 tier_not_allowed` on the Mistral Free tier — Mistral Large is withdrawn from Free entirely, and the admin *Limits* page is not a catalogue of tier-callable models. The Validator aborts every scenario until the **Mistral tier decision** (`docs/BACKLOG.md`) is made. The 2512 pin itself stays.
- **What works (8.5.3):** every agent audit payload (`doc_extract`, `coverage_check`, `settlement_estimate`, `output_check`) records `llm_call.requested_model` — the model passed to `provider.complete` — on success, parse-failure and provider-exception paths; omitted on the Adjuster demo-fixture path. Implemented by widening the Phase 8.3 capture object to `_shared.CapturedRequest(system, user, model)` with `attach_request`, which **deliberately overrides the prompt's preferred parallel-parameter design** (zero lines added to the already-oversized `_build_audit_payload` functions; recorded value equals sent value by construction). Additive locked-interface extension (bullet below). No frontend change (JSON viewer renders the key). Suite **347 passed, 0 failed, 7 skipped** (354 collected; +9 tests); discriminator-checked; `ruff` and `mypy` clean; installed version `0.8.5.3`.
- **What works (8.5.2, carried):** Validator and Adjuster defaults pinned from the moving alias `mistral-large-latest` to the dated release `mistral-large-2512` (`backend/settings.py`, `backend/settings.yaml.template`), with a pin-by-policy comment on `MistralProviderSettings`. Operational, not architectural — the locked "Mistral Large (Validator, Adjuster)" decision is unchanged. The pin is correct policy but did not fix the 403 (see *Demo status*).
- **What works (8.5.1, carried):** Phase 8.5's pending item is closed — the local test DB (`agentic_claims_test`, migrated to `0002_audit_human_agent`, `vector` enabled) is in place and the full backend suite runs green against it: **338 passed, 0 failed, 7 skipped** (345 collected — the long-quoted "338" is the non-skipped count; both figures are now recorded in the build log so the discrepancy is closed). Getting there required a hotfix. The first run gave 336 passed / 2 failed, both in `backend/tests/test_migration_0002.py`: `test_downgrade_constraint_rejects_existing_human_rows` performed `ALTER TABLE … DROP CONSTRAINT audit_log_agent_check` with no explicit transaction, relying on the implicit transaction that **Phase 8.4 abolished** when it moved connections to `autocommit=True` (`backend/db/connection.py:59`). Under autocommit the DROP committed instantly, the expected `CheckViolation` on the re-add still satisfied `pytest.raises` (so the test *passed* while destroying the schema), and the trailing `clean_db.rollback()` was a no-op — permanently stripping the constraint from the test DB and failing every subsequent run. CI never caught it because each CI run gets a fresh, disposable Postgres service container; only a *persistent* test DB — which Phase 8.5 had just introduced — makes the damage visible. The fix is test-layer only: the DDL surgery is wrapped in `clean_db.transaction()` (outermost `pytest.raises`, so the rollback runs before the exception is caught), the misleading no-op `rollback()` is removed, and a new post-assertion (`SELECT 1 FROM pg_constraint …`) verifies the constraint survived — turning the test into a genuine discriminator against DDL leaks rather than only against the re-add being refused. Proven by deliberately reinstating the pre-fix form and confirming it now fails. The local DB's missing constraint was repaired by hand. `ruff` clean repo-wide, `mypy` clean. No production code touched; no interface change. `/health` reports `version=0.8.5.1` (resolved via `importlib.metadata`, so the `pyproject.toml` bump is the only source).
- **What's next:** (1) Push `0.8.5.3` and wait for Render: `/health` → `0.8.5.3`. (2) **Part B — reset the deployed DB**, as one sequence with the verification: hostname-only check that `DATABASE_URL` ends in `.neon.tech` (stop otherwise); `uv run python -m backend.data.seed_claims --allow-truncate`; confirm 9 claims, three scenario tags at `received`. The CASCADE also clears `audit_log` (including runs `26a9bf4e-…`, `11f97ae8-…`) — accepted. Do **not** re-run `index_policy`. (3) Process the seeded `threshold_escalation` claim; the Validator abort is expected; confirm `coverage_check` `llm_call.requested_model = "mistral-large-2512"` and `doc_extract` `requested_model` = `model` = `claude-haiku-4-5-20251001`. Append to the Phase 8.5.3 build-log entry and report; follow-up commit. (4) Then the **Mistral tier decision** (Dermot) — which unblocks the Phase 8.4 / 8.5.2 seven-entry deployed verification. All other pending and future work lives in `docs/BACKLOG.md`.
