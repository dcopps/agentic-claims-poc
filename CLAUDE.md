...
- **What works:** Submission is decoupled from processing — `POST /api/claims` persists a claim with `status='received'` before any agent fires, and the orchestrator advances `claims.status` (`extracted` → `coverage_verified` → `estimated` → `guardrail_checked` → terminal) as the pipeline runs (non-fatal; the audit_log stays authoritative). Replay works: `POST /api/pipeline/replay/{claim_id}?variant=<name>` re-runs a claim under a configured variant (a `variants.yaml` registry ships `default`, `v2_strict_validator`, `v2_haiku_validator`) under a fresh correlation_id, never overwriting the prior run. A runs API reconstructs any past `PipelineResult` purely from the audit_log (`GET /api/runs/{correlation_id}`, `GET /api/claims/{claim_id}/runs`, `GET /api/runs/compare/{a}/{b}`). The Adjuster audit now carries full reasoning and the Validator audit reports the *actual* provider/model (so the Haiku variant proves real provider substitution). A functional React frontend wires submission, the claims list, process/replay buttons (with production-equivalent tooltips), an SSE live-progress strip, and a side-by-side comparison view — functional, not polished; Phase 6 is the polish. The three demo scenarios still pass; the new submit→run→replay→compare integration test passes. 277 backend + 13 frontend tests passing (6 skipped/gated); ruff and mypy clean across 95 source files; frontend tsc + eslint clean. `/health` reports `version=0.5.0`.
