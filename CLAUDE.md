...
- **Date:** 2026-09-14
- **Phase:** Phase 8.5.3 complete and deployed (`requested_model` audit key, point release `0.8.5.3`). Deployed DB reset done; deployed verification passed (run `56a0d5b2-a90d-43f3-866b-b5d35bd7f900`: `coverage_check` `llm_call.requested_model = "mistral-large-2512"` on the 403 — the runtime model is now proven from the audit row, not by elimination).
...
- **What's next:** (1) **Mistral tier decision** (Dermot) — options in `docs/BACKLOG.md` → *Mistral tier decision*; this is the sole demo blocker. (2) Once decided and deployed, run the Phase 8.4 / 8.5.2 seven-entry deployed verification in `docs/BACKLOG.md` → *Pending verifications*, resetting the Neon DB first (hostname gate, `seed_claims --allow-truncate`) because the 8.5.3 verification run left the seeded Northwood claim at `extracted`. (3) Phase 8.6 demo UI polish. All other pending and future work lives in `docs/BACKLOG.md`.
