...
- **Date:** 2026-06-26
- **Phase:** Phase 8.4 complete; deploy + deployed verification next.
- **What works:** Fixed a production-only silent audit-write data-loss bug found in Phase 8.3 rehearsal. On deployed `0.8.3`, three of four agents (`doc_parser`, `validator`, `adjuster`) showed ✓ in the UI but their audit rows were silently rolled back. Cause: `backend/db/connection.py` opened connections with `autocommit=False`; those three agents run a SELECT before the audit write, opening an implicit transaction that demoted `AuditWriter`'s `conn.transaction()` to a nested SAVEPOINT — durable only if the (never-committed) outer transaction committed, so `conn.close()` rolled the audit INSERT back. Guardrail and the orchestrator survived because they do no SQL before the audit write. The fix flips the connection to `autocommit=True`, so every `conn.transaction()` block is the top-level boundary and commits for real; the now-redundant post-`SET` `conn.commit()` and the two trailing script commits (`seed_claims.py`, `index_policy.py`) were removed. A full `open_connection` caller sweep confirmed no atomicity dependency breaks — every multi-statement write already wraps itself in `conn.transaction()`. New `backend/tests/test_audit_persistence.py` adds four regression tests (one per agent) using a two-connection design: write through the production `open_connection`, read the audit row through a *separate* connection — the only shape that catches the rollback (the prior suites read through the same connection that wrote the row, masking it). Discriminator validated locally: fixed → 4/4 pass; true original bug → 3 fail + Guardrail pass (reproduces the rehearsal); autocommit-off without the post-`SET` commit → 4/4 fail (proving the two edits are coupled). No interface change — all audit payload contracts unchanged. 335 backend (+4) + 36 frontend tests passing (7 skipped/gated); ruff and mypy clean. `/health` reports `version=0.8.4` once redeployed.
- **What's next:** Deploy to Render; confirm `/health` is `0.8.4`; re-run the threshold scenario and verify the audit log holds all seven entries (four agent steps + three orchestrator steps) with no "Audit entry not found" banners.
