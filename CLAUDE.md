...
- **Date:** 2026-09-15
- **Phase:** Phase 8.6 code complete and committed (Validator and Adjuster default to Claude Haiku; Mistral path retained as the `v1_mistral` variant; version `0.8.6`). **Deployed verification pending** — the sequence is in `docs/BACKLOG.md` → *Pending verifications* (it also discharges the Phase 8.4 / 8.5.2 seven-entry check).
- **Demo status: pending verification.** The Mistral blocker is resolved in code (no Mistral call on the default path). *Demo status* is cleared only once all three scripted scenarios land in their expected terminal states on the deployed app. A wrong terminal state halts the phase — no prompt or token retuning in 8.6.
...
- **What's next:** (1) Push → Render redeploy → Phase 8.6 deployed verification with Dermot (Neon reset, three scenarios, `v1_mistral` abort proof, one live `v2_strict_validator` replay), then a follow-up commit recording the outcome. (2) Phase 8.7 demo UI polish (`0.8.6 → 0.9.0`). All other pending and future work lives in `docs/BACKLOG.md`.
