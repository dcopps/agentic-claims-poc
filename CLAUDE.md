...
- **Date:** 2026-09-14
- **Phase:** Phase 8.5.2 complete locally (Mistral model pin, point release `0.8.5.2`); Render deploy + deployed verification pending (covers both Phase 8.5.2 and the Phase 8.4 carry-over).
- **What works (8.5.2):** The Validator and Adjuster defaults are pinned from the moving alias `mistral-large-latest` to the dated release `mistral-large-2512` (`backend/settings.py`, `backend/settings.yaml.template`). Reason: the Mistral account is on the Free tier, and Mistral re-pointed the `-latest` alias at a paid-tier Large release after May 2026 — the Validator aborted production run `26a9bf4e-44ce-49c9-bc99-aeaed33c9f8f` on 2026-09-14 with `403 tier_not_allowed`. `2512` is what the alias resolved to in May, so the pin restores the exact model the prototype was built against, and it remains on the Free tier's accessible list. A pin-by-policy comment on `MistralProviderSettings` records why the defaults are dated releases and must not be reverted to an alias. Operational, not architectural — the locked "Mistral Large (Validator, Adjuster)" decision is unchanged. Interface impact is a value change only: new `coverage_check` / `settlement_estimate` audit rows carry `llm_call.model = "mistral-large-2512"`; shapes unchanged. Suite **338 passed, 0 failed, 7 skipped** (345 collected, identical to 8.5.1); `ruff` and `mypy` clean; installed version `0.8.5.2`.
...
- **What's next:** (1) Dermot confirms no `LLM__MISTRAL__*` env var is set on Render (it would outrank the pinned default). (2) Deploy `0.8.5.2` to Render and run the combined deployed verification in `docs/BACKLOG.md` → *Pending verifications*: `/health = 0.8.5.2`; a **fresh** claim via the form's *Threshold escalation ($850k fire)* template (not the seeded Northwood row); Validator completes, `awaiting_human` reached, four agent panels filled with no *Audit entry not found* banners; seven audit entries with chain verified (the still-pending Phase 8.4 audit-persistence check); **both** `coverage_check` and `settlement_estimate` read `llm_call.model = "mistral-large-2512"`. Append the outcome to the Phase 8.5.2 build-log entry and report. (3) The eventual tier decision for when `2512` is deprecated is tracked in `docs/BACKLOG.md` → *Mistral tier upgrade path*; a *Startup model-access probe* is logged there too. All other pending and future work lives in `docs/BACKLOG.md`.
