...
sequenceDiagram
    ...
    participant V as Validator [Claude Haiku]
    participant A as Adjuster [Claude Haiku]
...
The diagram shows the prototype's default wiring. Since Phase 8.6 (15 September 2026) all four agents run on Anthropic by default: Mistral withdrew Mistral Large from its Free tier. The Mistral path is retained — the `v1_mistral` replay variant routes the Validator and Adjuster back to Mistral Large, and the production target is unchanged. The full interactive version (and three more diagrams, which show the target architecture) live in [`diagrams/`](diagrams/).

## What this demonstrates

- **A tiered model strategy** — Claude Sonnet (orchestration), Claude Haiku (fast extraction + output guardrails), Mistral Large (open-weight reasoning for the PII-sensitive coverage and settlement decisions, fine-tunable for the Adjuster). The deployed prototype currently runs single-vendor by default, with the Mistral path retained as a variant — see [`docs/design-decisions.md`](docs/design-decisions.md) §3.
...
- **Provider substitutability (DORA Article 28)** — an LLM Gateway mediates every model call, and each agent's provider is a settings selector. When Mistral Large left Mistral's Free tier, the Validator and Adjuster were moved to Claude Haiku by configuration, not code; the `v1_mistral` replay variant still routes them to Mistral. Every agent's audit entry records the *actual* provider and model, so the substitution is provable from the log.
...
cp .env.example .env                  # set DATABASE_URL (+ ANTHROPIC_API_KEY; MISTRAL_API_KEY for v1_mistral)
...
- `.env` is gitignored. `ANTHROPIC_API_KEY` is required for live agent runs; `MISTRAL_API_KEY` only for the `v1_mistral` variant (neither for the test suite, which mocks the LLM boundary).
...
- **Why an LLM Gateway?** A single mediated boundary makes provider substitution a configuration change (DORA Article 28), and is where prompt logging, cost attribution, and PII redaction live in production. The prototype's Gateway has been exercised across Anthropic and Mistral: the Validator and Adjuster ran on Mistral Large until Phase 8.6 and on Claude Haiku since, by a settings change.
- **Why Mistral *and* Claude?** Tiered cost/capability and provider diversity: Sonnet for orchestration, Haiku for cheap deterministic-ish extraction/guardrails, Mistral (open-weight, fine-tunable, tenant-hostable) for the PII-sensitive coverage and settlement decisions. *(15 September 2026: the deployed prototype defaults to Claude for all four agents after Mistral Large left Mistral's Free tier; the Mistral path is retained as the `v1_mistral` variant and remains the production target.)*
...
- **The audit log is the trusted record.** Every additive interface extension since Phase 4 preserves the property that the audit log alone reconstructs and explains any past decision. Concretely, the additive audit-payload extensions are:
  ...
  - Validator `coverage_check` (all four agents since Phase 8.6): `llm_call.provider` / `model` report the *actual* provider and model in use, not a hardcoded vendor — so a provider-substitution variant is provable from the log.
...
| LLMs | Anthropic public API by default; Mistral public API via the `v1_mistral` variant | Anthropic + Mistral models via Azure AI Foundry private endpoints |
